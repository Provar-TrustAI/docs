...
<CardGroup cols={2}>
  ...
  <Card title="Agent Version" icon="git-branch" href="/concepts/agent-versions">
    A snapshot of the agent under test, connected through one of four platforms. Running the same scenarios against two versions is how regressions get caught.
  </Card>
</CardGroup>
...
A new project doesn't start from a blank slate. **Add recommended evaluators** offers 11 curated LLM-judge presets — five **Quality** and six **Safety** — that become ordinary, fully editable evaluators once adopted. Nine are pass/fail; two (*Answer is complete* and *Accomplished the goal*) are scored 1–5 and pass at 3 or above, so those two report a score rather than a verdict. **Add recommended scenarios** offers 10 starter safety scenarios mapped to the OWASP Agentic AI Top 10, each paired with the evaluators that score it well.
