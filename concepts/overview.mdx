...
<CardGroup cols={2}>
  <Card title="Project" icon="sliders-horizontal" href="/concepts/projects">
    The workspace container — owns members, runtime connection config, and every Session, Scenario set, Evaluator, Evaluation, and Agent Version under it.
  </Card>
  <Card title="Session" icon="message-square" href="/concepts/sessions">
    One real interaction between a user and an agent — a sequence of Turns and Tool Calls. The raw material every Scenario set and Evaluation builds on.
  </Card>
  ...
  <Card title="Scenario set" icon="users-round" href="/concepts/scenarios">
    A curated, versioned collection of scenarios — test-case rows curated from Sessions or Turns. The stable test bed every Evaluation runs against, snapshotted so historical results stay reproducible.
  </Card>
  ...
  <Card title="Evaluation" icon="chart-line" href="/concepts/evaluations">
    A single run of one or more Evaluators across a Scenario set, scoped to an Agent Version. Scored, comparable, reviewable — the artifact regressions get caught in.
  </Card>
  <Card title="Agent Version" icon="git-branch" href="/concepts/agent-versions">
    A snapshot of the agent under test — prompt, model, tool set, deployment. Comparing two Agent Versions on the same Scenario set is the core regression-testing loop.
  </Card>
</CardGroup>

## How they connect

The loop is unchanged: a **Session** is reviewed into **Annotations**, curated into a **Scenario set**, scored by **Evaluators** in an **Evaluation**, and the results compared across two **Agent Versions**. The three new surfaces sit *on* that loop rather than replacing any noun:
...
flowchart LR
    subgraph loop["The evaluation loop"]
        ...
        D["<b>Scenario set</b><br/>curated scenarios<br/><i>test bed</i>"]
...
- **The loop** is the seven nouns, end to end, exactly as before. The **Scenario set** is still the curated test bed every Evaluation runs against — it lives on the **Scenarios** surface (more on that below).
- **Scenarios and the Playground feed and drive the loop.** A Playground run can be captured back into a Scenario set as a row, and a scenario that carries a persona and a goal can be simulated fresh at evaluation time instead of replaying stored turns.
- **The Trust Agent helps you run the whole loop.** It is the in-app assistant — it finds Sessions, builds Scenario sets, drafts Evaluators, and kicks off Evaluations on your behalf, pausing for your approval before it writes.
...
The **Trust Agent** is the default, flag-free in-app assistant on every project, reached at `/projects/:id/agent`. You ask it to find Sessions, build a Scenario set, draft an Evaluator, run an Evaluation, or analyze a regression — and it works in the open: it streams a transcript, renders the records it touches as clickable objects you can open in context, and gates every write behind a human-in-the-loop approval before it changes anything. It helps you run the loop; it does not replace any step of it.
...
**Scenarios** is the surface where your Scenario sets live — the persona-and-goal-shaped home of the loop's test beds. The list shows **Persona** and **Goal** columns, and each row is a **scenario**: one test case. The rename is reader-facing only — the API kept its legacy paths, and there is no scenarios endpoint; the precise mapping lives in the API note on the [Scenarios](/concepts/scenarios) page.
...
<CardGroup cols={2}>
  ...
  <Card title="Scenarios" icon="users" href="/concepts/scenarios">
    Persona-and-goal test sets. Build, generate, and curate Scenario sets, then simulate agents against them.
  </Card>
