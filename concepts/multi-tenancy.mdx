...
This page is a security and data-model concept, not a workflow. The claims here are architectural — properties of the schema, the shared tenant guard, and the repository layer, verified in the application's source and tests — rather than buttons you click. Where that distinction matters, the page says so.
...
{/* ACCURACY-AUDIT-PENDING: organization_id is NOT NULL on datasets/personas/evaluations/sessions/simulations + paddington tasks & audit log — schema property verified in models.py (datasets:67, personas:66, evaluations:61, simulations:90, sessions:69, paddington:174/764), not UI-drivable */}
...
{/* ACCURACY-AUDIT-PENDING: organization_id denormalized from the parent project at write time so list/index queries filter by tenant without joining through projects — code property (paddington/models.py tenancy-anchor comments; projects/tenancy.py resolve_project_org_id), not UI-drivable */}
...
{/* ACCURACY-AUDIT-PENDING: an agent session only sees/acts on the current org's project data; approved writes execute only within the user's org/project (tenant-scoped, server-anchored, approval-bound); cross-tenant agent task/audit id = 403/404 — observable property (paddington tasks/audit organization_id NOT NULL + test_tenant_denial.py; ADR-0019 Decision #3; release Trust Agent "secured write execution" #1683), verify by driving the agent on Org A on the release tag */}
