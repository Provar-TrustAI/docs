...
A **Repeat each test case** control (defaulting to 2) runs every test case multiple times, because an agent that passes once isn't an agent that passes reliably — a finished repeated run leads with a **Pass^K** reliability headline. For quick checks there's also one-off grading: **Evaluate in place** scores a single scenario or session where it lives, with no run added to the Evaluations list.
