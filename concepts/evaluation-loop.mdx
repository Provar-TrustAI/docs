...
icon: "refresh-cw"
description: "How Sessions become Scenario sets become Evaluations — the workflow Trust AI is built around."
---
...
flowchart TD
    ...
    S3["<b>3. Curate Scenario set</b><br/><i>versioned, snapshot-bound test bed</i>"]
...
The loop is not strictly linear — you'll often define an Evaluator before fully curating a Scenario set, or build a scenario set before having any Annotations. The arrows show the **dependency direction**, not a forced sequence.
...
Trust AI supports AWS Bedrock AgentCore today via a cross-account IAM role. Agentforce and Copilot Studio connectors are on the roadmap. The ingestion path is read-only; Trust AI never modifies the source runtime.
...
### 3. Curate a Scenario set

Annotated Sessions become the raw material for a **Scenario set** — a versioned, curated collection of test cases. The Scenario set is the stable test bed every Evaluation runs against. You filter, bulk-select, and curate the annotated Sessions into a Scenario set; the human judgment travels with each row.

A **scenario** — one row in the set — is a **snapshot** of the source Session, not a deep-link. Upstream edits don't change historical Evaluation results (ADR 0004). That's the core reproducibility guarantee.

**Read more:** [Scenarios](/concepts/scenarios)
...
An **Evaluation** is a single run: pick a Scenario set, pick one or more Evaluators, pick an Agent Version, and kick it off. Trust AI sends each scenario to the live agent (via the Project's runtime connection), collects responses, scores them with the Evaluators, and produces a per-row scored result.

A subtle but important pattern (ADR 0006): **the agent responses produced during an Evaluation are themselves Sessions**, structurally indistinguishable from production Sessions. They show up in your Sessions list tagged "from-eval," available for re-annotation, re-evaluation, or curation into the next Scenario set. The loop is genuinely a loop.
...
**Read more:** [Compare two Agent Versions](/how-to/compare-agent-versions) · [Agent Versions](/concepts/agent-versions)
...
<CardGroup cols={3}>
  <Card title="SI Consultant" icon="user-cog">
    **Touches all six steps** during a customer engagement. Sets up the Project, connects the runtime, leads the first wave of Annotations, scaffolds Scenario sets and Evaluators, runs the first Evaluations, then hands off recurring operations to the customer team.
  </Card>
  <Card title="QA Practitioner" icon="clipboard-check">
    **Lives in steps 2 and 3** — the Annotations queue and Scenario set curation. Spends real time per week here; their judgment is what makes every downstream Evaluator possible.
  </Card>
...
- **Investigate a regression.** A category of rows failed. Open the failing rows, read the reasoning, decide whether the agent regressed, the Evaluator is over-rejecting, or the scenario is no longer valid. Each of those points back into the loop at a different step.
...
In all three, the **next batch of Sessions ingested** completes the loop: new production interactions become new annotation candidates, which become new scenarios, which sharpen the next round of Evaluations.
...
- **[Scenarios](/concepts/scenarios)** — step 3 test beds
