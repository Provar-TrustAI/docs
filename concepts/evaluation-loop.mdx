...
Sessions arrive from three directions, and each one wears a **Source** chip telling you which: **Scenario** (simulated from a scenario run or evaluation), **Playground** (an unsaved Playground conversation), or **External** (synced from your connected runtime). Trust AI connects to **Salesforce Agentforce**, **AWS Bedrock AgentCore**, and **Claude Managed Agent** runtimes; syncing is read-only — Trust AI never modifies the source runtime.
...
Then make the judge earn your trust. An LLM-judge evaluator's **Calibrate** tab runs the exact production judge against scenarios you've already marked pass or fail and reports a plain-language agreement score, with disagreements listed first. Work the disagreements: refine a rule, re-check agreement, watch the calibration history climb. Judge verdicts are deterministic — the same transcript always gets the same verdict — so improvement you measure is improvement you keep.
